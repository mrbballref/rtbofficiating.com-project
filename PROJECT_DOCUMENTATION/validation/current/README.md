...
- `RTBO_PLATFORM_BUILD_FIRST_REFERENCE_VALIDATION_RULE.md` — mandatory create-complete-platform-first, run it, compare its finished UI/UX to the approved reference set, validate every line/system, lock it, and only then teach from that exact source
...
The assistant must create the complete code for each platform in an isolated validation workspace before teaching it. The finished platform must then be run and validated against its approved reference websites/applications, user-approved screenshots/files/assets, and the current RTBO requirements/design system. UI/UX validation is mandatory in addition to line-by-line source validation. The exact executed and locked source is the source that will later be taught.
