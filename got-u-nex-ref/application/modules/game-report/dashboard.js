(() => {
  ...
  const gamesUi = { section: 'reviews', editingId: '', query: '', page: 1, pageSize: 8, season: '', reportEditingId: '', reportGameId: '', reportQuery: '', reportPage: 1, reportPageSize: 8 };
  ...
  const emptyState = () => ({
    ...
    assignments: [], masterGames: [], schools: [], venues: [], payments: [], forms: [], resources: [], products: [], cart: [], conversations: [], reviews: [], gameEvaluations: [], gameReports: [], supportTickets: [], notifications: [], taxDeadlines: [], documentActivity: [], labVideos: [], labDiscussions: [], labAnnouncements: [], labFilms: [], labClipFolders: [], labFilmClips: [],
  ...
  function publishedGameRow(game) {
    ...
    return `<tr class="${reasons.length ? 'needs-editing' : ''}">
      ...
      <td data-label="Actions"><div class="published-list-actions"><button type="button" data-action="view-published-game" data-id="${esc(game.id)}">${icon('i-eye')}View</button><details><summary aria-label="More published game actions">${icon('i-chevron')}</summary><div><button type="button" data-action="open-game-report" data-id="${esc(game.id)}" ${gameReportIsEligible(game) || gameReportForGame(game.id) ? '' : 'disabled'}>${icon('i-form')}${gameReportForGame(game.id) ? 'Game Report' : 'Complete Game Report'}</button><button type="button" data-action="published-edit-game" data-id="${esc(game.id)}">${icon('i-edit')}Unpublish & Edit</button><button type="button" data-action="unpublish-published-game" data-id="${esc(game.id)}">${icon('i-close')}Unpublish</button><button type="button" data-action="download-published-assignment" data-id="${esc(game.id)}">${icon('i-download')}Download</button></div></details></div></td>
    </tr>`;
  }
  ...
  const DOCUMENT_CATEGORIES = [
    ...
    { key: 'game-reports', label: 'Game Reports', icon: 'i-form' },
  ...
  const GAME_REPORT_INCIDENTS = [
    ['injury', 'Injury'], ['equipment', 'Equipment Issue'], ['shot-clock', 'Shot Clock Issue'], ['scorebook', 'Scorebook Correction'],
    ['spectator', 'Spectator Disturbance'], ['bench-conduct', 'Bench Conduct Issue'], ['replay-review', 'Replay Review'],
    ['protest', 'Protest Filed'], ['security', 'Security Concern'], ['other', 'Other']
  ];
  const GAME_REPORT_FOUL_ROWS = [
    ['personal', 'Personal Fouls'], ['technical', 'Technical Fouls'], ['flagrant', 'Flagrant Fouls'],
    ['unsportsmanlike', 'Unsportsmanlike Conduct'], ['ejections', 'Ejections'], ['benchWarnings', 'Bench Warnings'], ['disqualifications', 'Disqualifications']
  ];
  const GAME_REPORT_PERIODS = [['q1','Q1'],['q2','Q2'],['q3','Q3'],['q4','Q4'],['ot','OT']];

  function normalizeGameReport(record = {}) {
    const scoreSide = (side) => ({ q1: '', q2: '', q3: '', q4: '', ot: '', final: '', ...((record.scores || {})[side] || {}) });
    const foulSide = (side) => ({ personal: '', technical: '', flagrant: '', unsportsmanlike: '', ejections: '', benchWarnings: '', disqualifications: '', ...((record.fouls || {})[side] || {}) });
    return {
      ...record,
      id: String(record.id || ''), linkedGameId: String(record.linkedGameId || ''), reportCode: String(record.reportCode || ''), status: String(record.status || 'Draft'),
      gameStatus: String(record.gameStatus || ''), gameDate: String(record.gameDate || ''), gameTime: String(record.gameTime || ''), venue: String(record.venue || '').trim(), court: String(record.court || '').trim(), competitionLeague: String(record.competitionLeague || '').trim(),
      homeTeam: String(record.homeTeam || '').trim(), awayTeam: String(record.awayTeam || '').trim(), scores: { home: scoreSide('home'), away: scoreSide('away') },
      crewChief: String(record.crewChief || '').trim(), referee: String(record.referee || '').trim(), umpire: String(record.umpire || '').trim(), alternateOfficial: String(record.alternateOfficial || '').trim(),
      scorekeeper: String(record.scorekeeper || '').trim(), timer: String(record.timer || '').trim(), siteAdministrator: String(record.siteAdministrator || '').trim(),
      actualStartTime: String(record.actualStartTime || ''), actualEndTime: String(record.actualEndTime || ''), totalDuration: String(record.totalDuration || ''),
      homeTimeouts: record.homeTimeouts === '' || record.homeTimeouts == null ? '' : Math.max(0, Number(record.homeTimeouts) || 0),
      awayTimeouts: record.awayTimeouts === '' || record.awayTimeouts == null ? '' : Math.max(0, Number(record.awayTimeouts) || 0),
      leadChanges: record.leadChanges === '' || record.leadChanges == null ? '' : Math.max(0, Number(record.leadChanges) || 0),
      overtimePlayed: Boolean(record.overtimePlayed), delayInGame: Boolean(record.delayInGame), delayDuration: String(record.delayDuration || '').trim(),
      fouls: { home: foulSide('home'), away: foulSide('away') }, incidents: Array.isArray(record.incidents) ? record.incidents.map(String) : [], otherIncident: String(record.otherIncident || '').trim(),
      narrative: String(record.narrative || '').trim(), crewChiefSignature: String(record.crewChiefSignature || '').trim(), submittedBy: String(record.submittedBy || '').trim(),
      submittedAt: String(record.submittedAt || ''), reviewedBy: String(record.reviewedBy || '').trim(), reviewStatus: String(record.reviewStatus || 'Pending Review'),
      createdAt: String(record.createdAt || ''), updatedAt: String(record.updatedAt || '')
    };
  }

  function gameReports() {
    state.gameReports = Array.isArray(state.gameReports) ? state.gameReports : [];
    return state.gameReports.map(normalizeGameReport).filter((record) => record.id);
  }

  function gameReportForGame(gameId = '') { return gameReports().find((record) => record.linkedGameId === gameId) || null; }

  function gameReportScheduledTimestamp(game) {
    const normalized = normalizeMasterGame(game);
    if (!normalized.date) return NaN;
    const time = /^\d{2}:\d{2}/.test(normalized.startTime) ? normalized.startTime.slice(0,5) : '23:59';
    const value = new Date(`${normalized.date}T${time}:00`);
    return value.getTime();
  }

  function gameReportIsEligible(game) {
    const normalized = normalizeMasterGame(game);
    if (normalized.status !== 'Published') return false;
    if (normalized.completedAt) return true;
    const timestamp = gameReportScheduledTimestamp(normalized);
    return Number.isFinite(timestamp) && timestamp <= Date.now();
  }

  function gameReportEligibilityText(game) {
    const normalized = normalizeMasterGame(game);
    if (normalized.status !== 'Published') return 'The game must be published before a report can be completed.';
    if (!normalized.date) return 'Add a scheduled game date before completing the report.';
    const timestamp = gameReportScheduledTimestamp(normalized);
    if (!Number.isFinite(timestamp)) return 'Add a valid game date and start time before completing the report.';
    return timestamp > Date.now() ? `Available after the scheduled start: ${fmtDate(normalized.date)}${normalized.startTime ? ` at ${calendarTime(normalized.startTime)}` : ''}.` : 'Ready for post-game submission.';
  }

  function gameReportCode(record) {
    if (record.reportCode) return record.reportCode;
    const date = String(record.gameDate || '').replace(/-/g,'') || 'UNDATED';
    const suffix = String(record.id || '').replace(/[^a-z0-9]/gi,'').slice(-6).toUpperCase() || 'REPORT';
    return `BGR-${date}-${suffix}`;
  }

  function gameReportGameLabel(record) {
    return [record.homeTeam, record.awayTeam].filter(Boolean).join(' vs. ') || record.venue || 'Basketball game report';
  }

  function gameReportContactName(school, roleValues = []) {
    if (!school) return '';
    const contacts = Array.isArray(school.contacts) ? school.contacts.map((contact) => normalizeSchoolContact(contact)) : [];
    const match = contacts.find((contact) => roleValues.includes(contact.role));
    return match ? contactFullName(match) : '';
  }

  function gameReportSeedFromGame(game, existing = {}) {
    const normalized = normalizeMasterGame(game || {});
    const homeSchool = publishedAssignmentSchool(normalized.homeTeam);
    const submittedBy = [state.profile.firstName, state.profile.lastName].filter(Boolean).join(' ').trim();
    return normalizeGameReport({
      ...existing,
      linkedGameId: normalized.id || existing.linkedGameId || '', gameDate: existing.gameDate || normalized.date, gameTime: existing.gameTime || normalized.startTime,
      venue: existing.venue || normalized.gymName || normalized.address, competitionLeague: existing.competitionLeague || normalized.conferenceLevel || normalized.school,
      homeTeam: existing.homeTeam || normalized.homeTeam, awayTeam: existing.awayTeam || normalized.awayTeam,
      crewChief: existing.crewChief || masterCrewName(normalized.refereeId), referee: existing.referee || masterCrewName(normalized.umpire1Id), umpire: existing.umpire || masterCrewName(normalized.umpire2Id),
      alternateOfficial: existing.alternateOfficial || masterCrewName(normalized.alternateId),
      siteAdministrator: existing.siteAdministrator || gameReportContactName(homeSchool, ['game-day-administrator','athletic-director']),
      submittedBy: existing.submittedBy || submittedBy || masterCrewName(normalized.refereeId)
    });
  }

  function gameReportPublishedOptions(selected = '') {
    const games = (state.masterGames || []).map(normalizeMasterGame).filter((game) => game.status === 'Published' && (gameReportIsEligible(game) || game.id === selected)).sort((a,b) => `${b.date}${b.startTime}`.localeCompare(`${a.date}${a.startTime}`));
    return `<option value="">Select a completed published game</option>${games.map((game) => option(game.id, selected, [game.date ? fmtDate(game.date) : '', [game.homeTeam, game.awayTeam].filter(Boolean).join(' vs. '), game.gymName].filter(Boolean).join(' — '))).join('')}`;
  }

  function gameReportNumberInput(name, value = '', options = '') {
    return `<input type="number" name="${esc(name)}" min="0" step="1" value="${value === '' ? '' : esc(value)}" ${options}>`;
  }

  function gameReportScoreTable(record) {
    const row = (side, label) => `<tr><th>${esc(label)}</th>${GAME_REPORT_PERIODS.map(([key]) => `<td>${gameReportNumberInput(`score__${side}__${key}`, record.scores[side][key], 'data-game-report-score')}</td>`).join('')}<td><input type="number" name="score__${side}__final" value="${esc(record.scores[side].final)}" readonly data-game-report-final></td></tr>`;
    return `<div class="game-report-scoreboard"><div class="game-report-matchup"><div><small>Home Team</small><strong data-game-report-home-name>${esc(record.homeTeam)}</strong><b data-game-report-home-final>${record.scores.home.final === '' ? '—' : esc(record.scores.home.final)}</b></div><span>–</span><div><small>Away Team</small><strong data-game-report-away-name>${esc(record.awayTeam)}</strong><b data-game-report-away-final>${record.scores.away.final === '' ? '—' : esc(record.scores.away.final)}</b></div></div><div class="game-report-table-scroll"><table><thead><tr><th>Team</th>${GAME_REPORT_PERIODS.map(([,label]) => `<th>${label}</th>`).join('')}<th>Final</th></tr></thead><tbody>${row('home', record.homeTeam || 'Home')}${row('away', record.awayTeam || 'Away')}</tbody></table></div><div class="game-report-winner"><strong>Winning Team</strong><span data-game-report-winner>Final scores are calculated from entered periods.</span><strong>Attendance</strong>${gameReportNumberInput('attendance', record.attendance || '')}</div></div>`;
  }

  function gameReportPersonnel(record) {
    const fields = [['crewChief','Crew Chief'],['referee','Referee'],['umpire','Umpire'],['alternateOfficial','Alternate Official'],['scorekeeper','Scorekeeper'],['timer','Timer'],['siteAdministrator','Site Administrator']];
    return `<div class="game-report-personnel">${fields.map(([name,label]) => `<label><span>${label}</span><input name="${name}" value="${esc(record[name])}"></label>`).join('')}</div>`;
  }

  function gameReportKeyData(record) {
    return `<div class="game-report-key-grid"><label><span>Actual Start Time</span><input type="time" name="actualStartTime" value="${esc(record.actualStartTime)}" data-game-report-time></label><label><span>Actual End Time</span><input type="time" name="actualEndTime" value="${esc(record.actualEndTime)}" data-game-report-time></label><label><span>Total Duration</span><input name="totalDuration" value="${esc(record.totalDuration)}" readonly data-game-report-duration></label><label><span>Timeouts Used — Home</span>${gameReportNumberInput('homeTimeouts', record.homeTimeouts)}</label><label><span>Timeouts Used — Away</span>${gameReportNumberInput('awayTimeouts', record.awayTimeouts)}</label><label><span>Lead Changes</span>${gameReportNumberInput('leadChanges', record.leadChanges)}</label><label class="game-report-check"><input type="checkbox" name="overtimePlayed" ${record.overtimePlayed ? 'checked' : ''}><span>Overtime Played</span></label><label class="game-report-check"><input type="checkbox" name="delayInGame" ${record.delayInGame ? 'checked' : ''}><span>Delay in Game</span></label><label><span>Delay Duration</span><input name="delayDuration" value="${esc(record.delayDuration)}"></label></div>`;
  }

  function gameReportFouls(record) {
    return `<div class="game-report-table-scroll"><table class="game-report-foul-table"><thead><tr><th>Category</th><th>Home</th><th>Away</th></tr></thead><tbody>${GAME_REPORT_FOUL_ROWS.map(([key,label]) => `<tr><th>${esc(label)}</th><td>${gameReportNumberInput(`foul__home__${key}`, record.fouls.home[key])}</td><td>${gameReportNumberInput(`foul__away__${key}`, record.fouls.away[key])}</td></tr>`).join('')}</tbody></table></div>`;
  }

  function gameReportIncidents(record) {
    return `<div class="game-report-incidents">${GAME_REPORT_INCIDENTS.map(([value,label]) => `<label><input type="checkbox" value="${esc(value)}" data-game-report-incident ${record.incidents.includes(value) ? 'checked' : ''}><span>${esc(label)}</span></label>`).join('')}<label class="is-full"><span>Other Incident Details</span><input name="otherIncident" value="${esc(record.otherIncident)}"></label></div>`;
  }

  function gameReportForm(record = {}) {
    const normalized = normalizeGameReport(record);
    const code = normalized.id ? gameReportCode(normalized) : '';
    return `<form class="basketball-game-report" data-form="game-report" novalidate><input type="hidden" name="id" value="${esc(normalized.id)}"><input type="hidden" name="homeTeam" value="${esc(normalized.homeTeam)}"><input type="hidden" name="awayTeam" value="${esc(normalized.awayTeam)}"><header class="game-report-brand"><span class="game-report-ball" aria-hidden="true"></span><div><h1>Basketball Game Report</h1><p>Post-Game Crew Chief Submission</p></div>${icon('i-form')}</header><section class="game-report-meta"><label class="is-wide"><span>Published Game <b>*</b></span><select name="linkedGameId" data-game-report-link required>${gameReportPublishedOptions(normalized.linkedGameId)}</select></label>${code ? `<label><span>Report ID</span><input value="${esc(code)}" readonly></label>` : ''}<label><span>Date</span><input type="date" name="gameDate" value="${esc(normalized.gameDate)}" readonly></label><label><span>Game Time</span><input type="time" name="gameTime" value="${esc(normalized.gameTime)}" readonly></label><label><span>Venue</span><input name="venue" value="${esc(normalized.venue)}"></label><label><span>Court</span><input name="court" value="${esc(normalized.court)}"></label><label><span>Competition / League</span><input name="competitionLeague" value="${esc(normalized.competitionLeague)}"></label><label><span>Game Status <b>*</b></span><select name="gameStatus" required><option value="">Select status</option>${['Final','Forfeit','Suspended','Canceled'].map((value) => option(value, normalized.gameStatus)).join('')}</select></label></section><div class="game-report-two-column"><section class="game-report-section"><header><b>1</b><h2>Match Summary</h2></header>${gameReportScoreTable(normalized)}</section><section class="game-report-section"><header><b>2</b><h2>Game Personnel</h2>${icon('i-user')}</header>${gameReportPersonnel(normalized)}</section></div><div class="game-report-two-column"><section class="game-report-section"><header><b>3</b><h2>Key Game Data</h2>${icon('i-clock')}</header>${gameReportKeyData(normalized)}</section><section class="game-report-section"><header><b>4</b><h2>Fouls and Discipline</h2>${icon('i-flag')}</header>${gameReportFouls(normalized)}</section></div><section class="game-report-section game-report-incident-section"><header><b>5</b><h2>Incident Checklist</h2><small>Check all that apply</small>${icon('i-info')}</header>${gameReportIncidents(normalized)}</section><section class="game-report-section"><header><b>6</b><h2>Narrative Report</h2>${icon('i-edit')}</header><p class="game-report-help">Summarize significant events, officiating notes, unusual incidents, and actions taken.</p><textarea name="narrative" rows="9">${esc(normalized.narrative)}</textarea></section><section class="game-report-section game-report-signoff"><header><b>7</b><h2>Sign-Off</h2>${icon('i-shield')}</header><div><label><span>Crew Chief Signature <b>*</b></span><input name="crewChiefSignature" value="${esc(normalized.crewChiefSignature)}"></label><label><span>Submitted By</span><input name="submittedBy" value="${esc(normalized.submittedBy)}"></label><label><span>Date / Time Submitted</span><input value="${normalized.submittedAt ? esc(fmtDateTime(normalized.submittedAt)) : ''}" readonly></label><label><span>Reviewed By</span><input name="reviewedBy" value="${esc(normalized.reviewedBy)}"></label><label><span>Review Status</span><select name="reviewStatus">${['Pending Review','Reviewed','Returned for Revision'].map((value) => option(value, normalized.reviewStatus)).join('')}</select></label></div></section><footer class="game-report-actions"><button type="button" data-action="back-to-game-reports">${icon('i-chevron')}Back to Reports</button><span></span><button type="submit" formnovalidate data-report-save-mode="draft">${icon('i-file')}Save Draft</button><button class="is-primary" type="submit" data-report-save-mode="submit">${icon('i-check')}${normalized.status === 'Submitted' ? 'Update Submitted Report' : 'Submit Game Report'}</button></footer></form>`;
  }

  function gameReportList() {
    const query = gamesUi.reportQuery.trim().toLowerCase();
    const published = (state.masterGames || []).map(normalizeMasterGame).filter((game) => game.status === 'Published');
    const reportByGame = new Map(gameReports().map((record) => [record.linkedGameId, record]));
    const rows = published.map((game) => ({ game, report: reportByGame.get(game.id) || null })).filter(({game, report}) => !query || [game.id, game.homeTeam, game.awayTeam, game.gymName, game.date, report?.status, report?.reportCode].filter(Boolean).join(' ').toLowerCase().includes(query)).sort((a,b) => `${b.game.date}${b.game.startTime}`.localeCompare(`${a.game.date}${a.game.startTime}`));
    const pageCount = Math.max(1, Math.ceil(rows.length / gamesUi.reportPageSize));
    gamesUi.reportPage = Math.min(Math.max(1, gamesUi.reportPage), pageCount);
    const pageItems = rows.slice((gamesUi.reportPage - 1) * gamesUi.reportPageSize, gamesUi.reportPage * gamesUi.reportPageSize);
    const submitted = gameReports().filter((record) => record.status === 'Submitted').length;
    const drafts = gameReports().filter((record) => record.status === 'Draft').length;
    const ready = published.filter((game) => gameReportIsEligible(game) && !reportByGame.has(game.id)).length;
    return `<section class="game-report-dashboard"><header><div><h1>Basketball Game Reports</h1><p>Complete post-game crew-chief reports from actual published games after their scheduled start time.</p></div><label>${icon('i-search')}<input type="search" value="${esc(gamesUi.reportQuery)}" data-game-report-search aria-label="Search game reports"></label></header><div class="game-report-metrics"><article><strong>${published.length}</strong><span>Published Games</span></article><article><strong>${ready}</strong><span>Ready for Report</span></article><article><strong>${drafts}</strong><span>Draft Reports</span></article><article><strong>${submitted}</strong><span>Submitted Reports</span></article></div>${pageItems.length ? `<div class="game-report-list"><div class="game-report-list-head"><span>Date & Time</span><span>Game</span><span>Venue</span><span>Crew Chief</span><span>Report Status</span><span>Action</span></div>${pageItems.map(({game,report}) => { const eligible = gameReportIsEligible(game); const status = report?.status || (eligible ? 'Ready' : 'Upcoming'); const actionLabel = report ? (report.status === 'Submitted' ? 'View / Update' : 'Continue Draft') : 'Complete Report'; return `<article><div><strong>${game.date ? esc(fmtDate(game.date)) : ''}</strong><small>${game.startTime ? esc(calendarTime(game.startTime)) : ''}</small></div><div><strong>${esc([game.homeTeam, game.awayTeam].filter(Boolean).join(' vs. '))}</strong><small>${esc([game.level, game.gender, game.sport].filter(Boolean).join(' · '))}</small></div><div><strong>${esc(game.gymName)}</strong><small>${esc(game.address)}</small></div><div><strong>${esc(masterCrewName(game.refereeId))}</strong><small>Crew Chief</small></div><div><span class="game-report-status is-${esc(status.toLowerCase().replace(/[^a-z0-9]+/g,'-'))}">${esc(status)}</span><small>${report?.submittedAt ? esc(fmtDateTime(report.submittedAt)) : esc(gameReportEligibilityText(game))}</small></div><button type="button" data-action="open-game-report" data-id="${esc(game.id)}" ${eligible || report ? '' : 'disabled'}>${icon(report ? 'i-edit' : 'i-form')}${esc(actionLabel)}</button></article>`; }).join('')}</div><nav class="game-report-pagination"><button type="button" data-action="game-report-page" data-page="${gamesUi.reportPage - 1}" ${gamesUi.reportPage <= 1 ? 'disabled' : ''}>Previous</button><span>Page ${gamesUi.reportPage} of ${pageCount}</span><button type="button" data-action="game-report-page" data-page="${gamesUi.reportPage + 1}" ${gamesUi.reportPage >= pageCount ? 'disabled' : ''}>Next</button></nav>` : `<div class="game-report-empty">${icon('i-form')}<h2>No published games are available.</h2><p>Publish an actual game assignment before creating a post-game report.</p><button type="button" data-action="open-published-games-from-reports">Open Published Games</button></div>`}</section>`;
  }

  function populateGameReportFromGame(form, gameId) {
    const game = (state.masterGames || []).map(normalizeMasterGame).find((item) => item.id === gameId);
    if (!form || !game) return;
    const seed = gameReportSeedFromGame(game);
    const set = (name, value) => { const input = form.elements.namedItem(name); if (input) input.value = value ?? ''; };
    ['gameDate','gameTime','venue','competitionLeague','homeTeam','awayTeam','crewChief','referee','umpire','alternateOfficial','siteAdministrator','submittedBy'].forEach((name) => set(name, seed[name]));
    const homeName = form.querySelector('[data-game-report-home-name]'); const awayName = form.querySelector('[data-game-report-away-name]');
    if (homeName) homeName.textContent = seed.homeTeam || ''; if (awayName) awayName.textContent = seed.awayTeam || '';
    form.querySelectorAll('[data-game-report-score]').forEach((input) => { input.value = ''; });
    updateGameReportCalculatedFields(form);
  }

  function updateGameReportCalculatedFields(form) {
    if (!form) return;
    const total = (side) => GAME_REPORT_PERIODS.reduce((sum,[key]) => sum + (Number(form.elements.namedItem(`score__${side}__${key}`)?.value) || 0), 0);
    const home = total('home'); const away = total('away');
    const homeFinal = form.elements.namedItem('score__home__final'); const awayFinal = form.elements.namedItem('score__away__final');
    if (homeFinal) homeFinal.value = String(home); if (awayFinal) awayFinal.value = String(away);
    const homeText = form.querySelector('[data-game-report-home-final]'); const awayText = form.querySelector('[data-game-report-away-final]');
    if (homeText) homeText.textContent = String(home); if (awayText) awayText.textContent = String(away);
    const winner = form.querySelector('[data-game-report-winner]');
    if (winner) { const homeName = form.elements.namedItem('homeTeam')?.value || 'Home team'; const awayName = form.elements.namedItem('awayTeam')?.value || 'Away team'; winner.textContent = home === away ? (home || away ? 'Tie score — verify overtime or final totals.' : 'Final scores are calculated from entered periods.') : `${home > away ? homeName : awayName} — ${Math.max(home,away)} to ${Math.min(home,away)}`; }
    const start = form.elements.namedItem('actualStartTime')?.value; const end = form.elements.namedItem('actualEndTime')?.value; const duration = form.elements.namedItem('totalDuration');
    if (duration) { if (start && end) { let [sh,sm] = start.split(':').map(Number); let [eh,em] = end.split(':').map(Number); let minutes = (eh * 60 + em) - (sh * 60 + sm); if (minutes < 0) minutes += 1440; duration.value = `${Math.floor(minutes/60)}:${String(minutes%60).padStart(2,'0')}`; } else duration.value = ''; }
  }

  function gameReportHtml(record) {
    const scores = record.scores;
    const foulRows = GAME_REPORT_FOUL_ROWS.map(([key,label]) => `<tr><td>${esc(label)}</td><td>${esc(record.fouls.home[key])}</td><td>${esc(record.fouls.away[key])}</td></tr>`).join('');
    const incidentLabels = GAME_REPORT_INCIDENTS.filter(([value]) => record.incidents.includes(value)).map(([,label]) => label);
    return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>${esc(gameReportCode(record))} Basketball Game Report</title><style>body{font-family:Arial,sans-serif;background:#eee;margin:0;padding:30px;color:#101828}.page{max-width:1000px;margin:auto;background:#fff;padding:28px;border-radius:16px;box-shadow:0 8px 28px #0002}header{display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #174f91;padding-bottom:16px}h1{font-size:34px;margin:0}h2{margin:26px 0 8px;background:#0b4b91;color:#fff;padding:9px 12px;font-size:18px}dl{display:grid;grid-template-columns:180px 1fr;gap:7px 16px}dt{font-weight:bold}dd{margin:0}table{width:100%;border-collapse:collapse}th,td{border:1px solid #c8d1dc;padding:8px;text-align:left}.score{font-size:25px;color:#0b4b91;font-weight:bold}.narrative{white-space:pre-wrap;line-height:1.45;border:1px solid #c8d1dc;padding:14px}.sign{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}</style></head><body><main class="page"><header><div><h1>Basketball Game Report</h1><p>Post-Game Crew Chief Submission</p></div><strong>${esc(gameReportCode(record))}</strong></header><dl><dt>Date / Time</dt><dd>${esc(record.gameDate ? fmtDate(record.gameDate) : '')} ${esc(record.gameTime ? calendarTime(record.gameTime) : '')}</dd><dt>Venue / Court</dt><dd>${esc([record.venue,record.court].filter(Boolean).join(' · '))}</dd><dt>Competition / League</dt><dd>${esc(record.competitionLeague)}</dd><dt>Game Status</dt><dd>${esc(record.gameStatus)}</dd></dl><h2>Match Summary</h2><p class="score">${esc(record.homeTeam)} ${esc(scores.home.final)} — ${esc(scores.away.final)} ${esc(record.awayTeam)}</p><table><thead><tr><th>Team</th>${GAME_REPORT_PERIODS.map(([,label]) => `<th>${label}</th>`).join('')}<th>Final</th></tr></thead><tbody>${['home','away'].map((side) => `<tr><td>${esc(side === 'home' ? record.homeTeam : record.awayTeam)}</td>${GAME_REPORT_PERIODS.map(([key]) => `<td>${esc(scores[side][key])}</td>`).join('')}<td>${esc(scores[side].final)}</td></tr>`).join('')}</tbody></table><h2>Game Personnel</h2><dl><dt>Crew Chief</dt><dd>${esc(record.crewChief)}</dd><dt>Referee</dt><dd>${esc(record.referee)}</dd><dt>Umpire</dt><dd>${esc(record.umpire)}</dd><dt>Alternate</dt><dd>${esc(record.alternateOfficial)}</dd><dt>Scorekeeper</dt><dd>${esc(record.scorekeeper)}</dd><dt>Timer</dt><dd>${esc(record.timer)}</dd><dt>Site Administrator</dt><dd>${esc(record.siteAdministrator)}</dd></dl><h2>Key Game Data</h2><dl><dt>Actual Time</dt><dd>${esc(record.actualStartTime)} – ${esc(record.actualEndTime)} (${esc(record.totalDuration)})</dd><dt>Timeouts</dt><dd>Home ${esc(record.homeTimeouts)} / Away ${esc(record.awayTimeouts)}</dd><dt>Lead Changes</dt><dd>${esc(record.leadChanges)}</dd><dt>Overtime</dt><dd>${record.overtimePlayed ? 'Yes' : 'No'}</dd><dt>Delay</dt><dd>${record.delayInGame ? `Yes${record.delayDuration ? ` — ${esc(record.delayDuration)}` : ''}` : 'No'}</dd></dl><h2>Fouls and Discipline</h2><table><thead><tr><th>Category</th><th>Home</th><th>Away</th></tr></thead><tbody>${foulRows}</tbody></table><h2>Incidents</h2><p>${esc([...incidentLabels, record.otherIncident].filter(Boolean).join(', ') || 'No incidents recorded.')}</p><h2>Narrative Report</h2><div class="narrative">${esc(record.narrative)}</div><h2>Sign-Off</h2><div class="sign"><div><strong>Crew Chief Signature</strong><p>${esc(record.crewChiefSignature)}</p></div><div><strong>Submitted By</strong><p>${esc(record.submittedBy)}</p></div><div><strong>Submitted</strong><p>${esc(record.submittedAt ? fmtDateTime(record.submittedAt) : '')}</p></div></div></main></body></html>`;
  }
  ...
  function normalizeGameEvaluation(record = {}) {
    ...
    return {
      ...
      officialName: String(record.officialName || '').trim(), evaluatorName: String(record.evaluatorName || '').trim(), evaluationDate: String(record.evaluationDate || ''),
      ...
      generalComments: String(record.generalComments || '').trim(), strengths: String(record.strengths || '').trim(), areasForImprovement: String(record.areasForImprovement || '').trim(),
      overallSummary: String(record.overallSummary || '').trim(), evaluatorSignature: String(record.evaluatorSignature || '').trim(), officialAcknowledgement: String(record.officialAcknowledgement || '').trim(),
      signatureDate: String(record.signatureDate || ''), tracking: record.tracking && typeof record.tracking === 'object' ? record.tracking : {},
  ...
  function gameEvaluationTasks() {
    const completedCount = gameEvaluations().filter(gameEvaluationIsComplete).length;
    const reportCount = gameReports().filter((record) => record.status === 'Submitted').length;
    return `<nav class="games-task-nav" aria-label="Games tasks"><button type="button" class="${gamesUi.section === 'reviews' ? 'is-active' : ''}" data-action="set-games-section" data-section="reviews">${icon('i-star')}My Reviews <span>${completedCount}</span></button><button type="button" class="${gamesUi.section === 'evaluation' ? 'is-active' : ''}" data-action="set-games-section" data-section="evaluation">${icon('i-form')}Evaluation Form</button><button type="button" class="${gamesUi.section === 'history' ? 'is-active' : ''}" data-action="set-games-section" data-section="history">${icon('i-file')}Saved Evaluations <span>${gameEvaluations().length}</span></button><button type="button" class="${['reports','report'].includes(gamesUi.section) ? 'is-active' : ''}" data-action="set-games-section" data-section="reports">${icon('i-trophy')}Game Reports <span>${reportCount}</span></button></nav>`;
  }
  ...
  function gameEvaluationBasicInfo(record) {
    return `<section class="evaluation-section evaluation-basic-info"><header><h2>1. Header / Basic Info</h2></header><div class="evaluation-basic-grid"><label class="is-wide"><span>Saved Game</span><select name="linkedGameId" data-game-evaluation-link>${gameEvaluationGameOptions(record.linkedGameId)}</select></label><label><span>Saved Official</span><select name="officialId" data-game-evaluation-official>${gameEvaluationRosterOptions(record.officialId)}</select></label><label><span>Official's Name <b>*</b></span><input name="officialName" value="${esc(record.officialName)}" required></label><label><span>Saved Evaluator</span><select name="evaluatorId" data-game-evaluation-evaluator>${gameEvaluationRosterOptions(record.evaluatorId, 'Select saved evaluator')}</select></label><label><span>Evaluator's Name <b>*</b></span><input name="evaluatorName" value="${esc(record.evaluatorName)}" required></label><label><span>Date of Evaluation <b>*</b></span><input type="date" name="evaluationDate" value="${esc(record.evaluationDate)}" required></label><label><span>Site of Game</span><input name="site" value="${esc(record.site)}"></label><label><span>Home Team</span><input name="homeTeam" value="${esc(record.homeTeam)}"></label><label><span>Visiting Team</span><input name="visitingTeam" value="${esc(record.visitingTeam)}"></label><label><span>Level / Gender</span><input name="levelGender" value="${esc(record.levelGender)}"></label><label class="is-wide"><span>Partner(s)</span><input name="partners" value="${esc(record.partners)}"></label></div></section>`;
  }
  ...
  function gameEvaluationIsComplete(record) {
    return Boolean(record && record.overallScore > 0 && record.evaluatorSignature && record.signatureDate && Object.keys(record.scores || {}).length);
  }

  function gameEvaluationSeasonKey(value = '') {
    if (!value) return '';
    const date = new Date(`${value}T12:00:00`);
    if (Number.isNaN(date.getTime())) return '';
    const year = date.getFullYear();
  ...
  function gameEvaluationReviewScope() {
    const completed = gameEvaluations().filter(gameEvaluationIsComplete).sort((a,b) => `${b.evaluationDate}${b.updatedAt}`.localeCompare(`${a.evaluationDate}${a.updatedAt}`));
    ...
    const profileRecords = normalizedProfile ? completed.filter((record) => record.officialName.toLowerCase() === normalizedProfile) : [];
    const records = profileRecords.length ? profileRecords : completed;
    const seasons = [...new Set(records.map((record) => gameEvaluationSeasonKey(record.evaluationDate)).filter(Boolean))].sort((a,b) => b.localeCompare(a));
    if (!gamesUi.season || (gamesUi.season !== 'all' && !seasons.includes(gamesUi.season))) gamesUi.season = seasons[0] || 'all';
    const filtered = gamesUi.season === 'all' ? records : records.filter((record) => gameEvaluationSeasonKey(record.evaluationDate) === gamesUi.season);
    const officialNames = [...new Set(filtered.map((record) => record.officialName).filter(Boolean))];
    return { completed, records, filtered, seasons, profileName, profileMatched: profileRecords.length > 0, officialName: officialNames.length === 1 ? officialNames[0] : profileRecords.length ? profileName : '' };
  }

  function gameEvaluationMean(values = []) {
    const numbers = values.map(Number).filter((value) => Number.isFinite(value) && value > 0);
    return numbers.length ? numbers.reduce((sum, value) => sum + value, 0) / numbers.length : 0;
  }

  function gameEvaluationGrade(value) {
    if (!value) return '—';
  ...
  function gameEvaluationStars(value) {
    const rounded = Math.max(0, Math.min(5, Math.round(Number(value) || 0)));
    return `<span class="evaluation-review-stars" aria-label="${Number(value || 0).toFixed(2)} out of 5">${Array.from({length:5}, (_, index) => `<i class="${index < rounded ? 'is-active' : ''}">★</i>`).join('')}</span>`;
  }

  function gameEvaluationCategoryMetrics(records = []) {
    const specs = [
      ['Mechanics', (key) => key.startsWith('mechanics.')],
      ['Positioning', (key) => key === 'application.position' || key === 'mechanics.positioning' || key.startsWith('coverage.')],
      ['Communication', (key) => key === 'presence.communication' || key.startsWith('crewInteraction.') || /Communication$/i.test(key)],
      ['Professionalism', (key) => ['presence.professionalism','presence.attitude','presence.demeanor','comportment.uniform','comportment.grooming','comportment.poise','comportment.dignified'].includes(key)],
      ['Rules Knowledge', (key) => key.startsWith('knowledge.') || key.startsWith('rulesKnowledge.')]
    ];
    return specs.map(([label, matches]) => {
      const values = [];
      records.forEach((record) => Object.entries(record.scores || {}).forEach(([key,value]) => { if (matches(key)) values.push(Number(value)); }));
      return { label, value: gameEvaluationMean(values) };
    });
  }

  function gameEvaluationRadar(metrics) {
    const centerX = 158, centerY = 132, radius = 88;
    ...
    const rings = [1,2,3,4,5].map((step) => `<polygon points="${metrics.map((_,index) => point(index, step / 5).map((n) => n.toFixed(1)).join(',')).join(' ')}"></polygon>`).join('');
    const axes = metrics.map((_,index) => { const [x,y] = point(index,1); return `<line x1="${centerX}" y1="${centerY}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}"></line>`; }).join('');
    const dataPoints = metrics.map((metric,index) => point(index, Math.max(0, Math.min(5, metric.value)) / 5).map((n) => n.toFixed(1)).join(',')).join(' ');
    const labels = metrics.map((metric,index) => { const [x,y] = point(index,1.28); const anchor = x < centerX - 8 ? 'end' : x > centerX + 8 ? 'start' : 'middle'; return `<text x="${x.toFixed(1)}" y="${y.toFixed(1)}" text-anchor="${anchor}"><tspan x="${x.toFixed(1)}">${esc(metric.label)}</tspan><tspan x="${x.toFixed(1)}" dy="15">${metric.value ? metric.value.toFixed(2) : '—'}</tspan></text>`; }).join('');
    return `<svg class="evaluation-review-radar" viewBox="0 0 316 264" role="img" aria-label="Performance by category">${rings}${axes}<polygon class="evaluation-review-radar-data" points="${dataPoints}"></polygon>${metrics.map((metric,index) => { const [x,y] = point(index, Math.max(0, Math.min(5, metric.value)) / 5); return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4"></circle>`; }).join('')}${labels}</svg>`;
  }

  function gameEvaluationTrend(records = []) {
    const points = [...records].sort((a,b) => `${a.evaluationDate}${a.createdAt}`.localeCompare(`${b.evaluationDate}${b.createdAt}`)).slice(-8);
    if (!points.length) return `<div class="evaluation-review-empty-chart">Completed evaluations will create the performance trend.</div>`;
    const width = 620, height = 190, left = 42, right = 18, top = 18, bottom = 38;
    const x = (index) => points.length === 1 ? width / 2 : left + index * ((width - left - right) / (points.length - 1));
    const y = (score) => top + (5 - Math.max(1,Math.min(5,score))) * ((height - top - bottom) / 4);
    const line = points.map((record,index) => `${index ? 'L' : 'M'}${x(index).toFixed(1)},${y(record.overallScore).toFixed(1)}`).join(' ');
    const benchmarkY = y(4);
    const grid = [3,4,5].map((score) => `<g><line x1="${left}" y1="${y(score).toFixed(1)}" x2="${width-right}" y2="${y(score).toFixed(1)}"></line><text x="4" y="${(y(score)+4).toFixed(1)}">${score.toFixed(2)}</text></g>`).join('');
    const dots = points.map((record,index) => { const date = record.evaluationDate ? new Date(`${record.evaluationDate}T12:00:00`) : null; const label = date && !Number.isNaN(date.getTime()) ? date.toLocaleDateString(undefined,{month:'short',day:'numeric'}) : ''; return `<g><circle cx="${x(index).toFixed(1)}" cy="${y(record.overallScore).toFixed(1)}" r="4"></circle><text class="score-label" x="${x(index).toFixed(1)}" y="${(y(record.overallScore)-10).toFixed(1)}" text-anchor="middle">${record.overallScore.toFixed(2)}</text><text class="date-label" x="${x(index).toFixed(1)}" y="${height-10}" text-anchor="middle">${esc(label)}</text></g>`; }).join('');
    return `<svg class="evaluation-review-trend" viewBox="0 0 ${width} ${height}" role="img" aria-label="Evaluation performance trend">${grid}<line class="benchmark" x1="${left}" y1="${benchmarkY.toFixed(1)}" x2="${width-right}" y2="${benchmarkY.toFixed(1)}"></line><path d="${line}"></path>${dots}</svg><div class="evaluation-review-chart-key"><span><i></i>Overall Average</span><span><i class="is-benchmark"></i>Standard Benchmark (4.00)</span></div>`;
  }

  function gameEvaluationSeasonRank(scope, average) {
    if (!scope.officialName || !gamesUi.season || gamesUi.season === 'all') return { rank: 0, total: 0, grade: gameEvaluationGrade(average) };
    const seasonRecords = scope.completed.filter((record) => gameEvaluationSeasonKey(record.evaluationDate) === gamesUi.season);
    const grouped = new Map();
    seasonRecords.forEach((record) => { const key = record.officialName.toLowerCase(); if (!grouped.has(key)) grouped.set(key, { name: record.officialName, scores: [] }); grouped.get(key).scores.push(record.overallScore); });
    const ranked = [...grouped.values()].map((item) => ({ ...item, average: gameEvaluationMean(item.scores) })).sort((a,b) => b.average - a.average);
    const index = ranked.findIndex((item) => item.name.toLowerCase() === scope.officialName.toLowerCase());
    return { rank: index >= 0 ? index + 1 : 0, total: ranked.length, grade: gameEvaluationGrade(average) };
  }

  function gameEvaluationImprovement(scope, average) {
    if (!average || !gamesUi.season || gamesUi.season === 'all') return { value: 0, available: false, previousSeason: '' };
    const start = Number(gamesUi.season.split('-')[0]);
    if (!Number.isFinite(start)) return { value: 0, available: false, previousSeason: '' };
    const previousSeason = `${start - 1}-${start}`;
    let previous = scope.records.filter((record) => gameEvaluationSeasonKey(record.evaluationDate) === previousSeason);
    if (scope.officialName) previous = previous.filter((record) => record.officialName.toLowerCase() === scope.officialName.toLowerCase());
    const previousAverage = gameEvaluationMean(previous.map((record) => record.overallScore));
    return { value: previousAverage ? average - previousAverage : 0, available: Boolean(previousAverage), previousSeason };
  }

  function gameEvaluationReviewsDashboard() {
    const scope = gameEvaluationReviewScope();
    const records = scope.filtered;
    if (!records.length) {
      return `<section class="evaluation-review-dashboard"><header class="evaluation-review-header"><div>${icon('i-star')}<span><h1>Evaluations — My Reviews</h1><p>Completed referee evaluations appear here automatically.</p></span></div>${scope.seasons.length ? `<label>Season<select data-game-evaluation-season><option value="all">All seasons</option>${scope.seasons.map((season) => option(season,gamesUi.season,`Season ${season}`)).join('')}</select></label>` : ''}</header><div class="game-evaluation-empty">${icon('i-star')}<h2>No completed evaluations are available.</h2><p>Submit the linked Basketball Referee Evaluation Form to populate averages, category ratings, trends, and evaluator comments.</p><button type="button" data-action="set-games-section" data-section="evaluation">Open Evaluation Form</button></div></section>`;
    }
    const average = gameEvaluationMean(records.map((record) => record.overallScore));
    const latest = records[0];
    const metrics = gameEvaluationCategoryMetrics(records);
    const rank = gameEvaluationSeasonRank(scope, average);
    const improvement = gameEvaluationImprovement(scope, average);
    const recent = records.slice(0,5);
    const latestComment = latest.overallSummary || latest.generalComments || latest.strengths || '';
    const evaluatorInitials = latest.evaluatorName.split(/\s+/).filter(Boolean).slice(0,2).map((part) => part[0]).join('').toUpperCase();
    const contextText = scope.profileMatched ? `Showing completed evaluations for ${scope.profileName}.` : scope.officialName ? `Showing completed evaluations for ${scope.officialName}.` : 'Showing all completed evaluations saved in this dashboard.';
    return `<section class="evaluation-review-dashboard"><header class="evaluation-review-header"><div>${icon('i-star')}<span><h1>Evaluations — My Reviews</h1><p>Track performance. Elevate your game. ${esc(contextText)}</p></span></div><label>Season<select data-game-evaluation-season><option value="all" ${gamesUi.season === 'all' ? 'selected' : ''}>All seasons</option>${scope.seasons.map((season) => option(season,gamesUi.season,`Season ${season}`)).join('')}</select></label></header><div class="evaluation-review-metrics"><article class="is-primary"><span>Overall Average</span><strong>${average.toFixed(2)}</strong>${gameEvaluationStars(average)}<small>${average >= 4 ? 'Above Standard' : average >= 3 ? 'Meets Standard' : 'Development Opportunity'}</small></article><article><span>Total Evaluations</span><strong>${records.length}</strong><small>${records.length === 1 ? 'Completed evaluation' : 'Completed evaluations'}</small></article><article><span>Latest Evaluation</span><strong>${latest.overallScore.toFixed(2)}</strong><small>${latest.evaluationDate ? esc(fmtDate(latest.evaluationDate)) : 'Date not entered'}</small></article><article><span>Season Rank</span><strong class="is-grade">${esc(rank.grade)}</strong><small>${rank.rank ? `#${rank.rank} of ${rank.total} evaluated officials` : 'Rank requires one identified official and season'}</small></article><article><span>Improvement</span><strong class="${improvement.available && improvement.value >= 0 ? 'is-positive' : improvement.available ? 'is-negative' : ''}">${improvement.available ? `${improvement.value >= 0 ? '+' : ''}${improvement.value.toFixed(2)}` : '—'}</strong><small>${improvement.available ? `vs. season ${esc(improvement.previousSeason)}` : 'No prior-season evaluations'}</small></article></div><div class="evaluation-review-grid"><section class="evaluation-review-panel evaluation-review-performance"><header><h2>Performance by Category</h2></header><div class="evaluation-review-performance-body">${gameEvaluationRadar(metrics)}<div class="evaluation-review-category-list">${metrics.map((metric) => `<article><span>${esc(metric.label)}</span><strong>${metric.value ? metric.value.toFixed(2) : '—'}</strong>${gameEvaluationStars(metric.value)}</article>`).join('')}<button type="button" data-action="set-games-section" data-section="history">View Category Breakdown ${icon('i-chevron')}</button></div></div></section><section class="evaluation-review-panel evaluation-review-recent"><header><h2>Recent Evaluations</h2><button type="button" data-action="set-games-section" data-section="history">View All</button></header><div>${recent.map((record) => `<button type="button" data-action="edit-game-evaluation" data-id="${esc(record.id)}"><span class="evaluation-review-ball">${icon('i-star')}</span><span><strong>${esc(gameEvaluationGameLabel(record))}</strong><small>${[record.levelGender, record.evaluationDate ? fmtDate(record.evaluationDate) : ''].filter(Boolean).map(esc).join(' · ')}</small></span><b>${record.overallScore.toFixed(2)}</b>${gameEvaluationStars(record.overallScore)}<em>Completed</em>${icon('i-chevron')}</button>`).join('')}</div></section><section class="evaluation-review-panel evaluation-review-trend-panel"><header><h2>Performance Trend</h2><span title="Based only on completed evaluation records">${icon('i-info')}</span></header>${gameEvaluationTrend(records)}</section><section class="evaluation-review-panel evaluation-review-comments"><header><h2>Evaluator Comments (Latest)</h2></header><blockquote>${latestComment ? esc(latestComment) : 'No written evaluator comments were entered for this completed evaluation.'}</blockquote><footer><span class="evaluation-review-avatar">${esc(evaluatorInitials || '—')}</span><span><strong>${esc(latest.evaluatorName)}</strong><small>Evaluator</small></span><time>${latest.evaluationDate ? esc(fmtDate(latest.evaluationDate)) : ''}</time><b>${latest.overallScore.toFixed(2)}</b>${gameEvaluationStars(latest.overallScore)}</footer></section></div></section>`;
  }
  ...
  function gameEvaluationForm(record = {}) {
    ...
    return `<form class="game-evaluation-form" data-form="game-evaluation" novalidate><input type="hidden" name="id" value="${esc(normalized.id)}">${gameEvaluationHeader(normalized)}${gameEvaluationBasicInfo(normalized)}${gameEvaluationRatingLegend()}<div class="evaluation-main-grid"><div class="evaluation-table-stack">${tableGroups.map((group) => gameEvaluationCriterionMarkup(group, normalized)).join('')}</div><section class="evaluation-section evaluation-panels"><header><h2>4. Evaluation Panels</h2></header><div>${panels.map((group) => gameEvaluationCriterionMarkup(group, normalized)).join('')}</div></section></div><section class="evaluation-section evaluation-general-comments"><header><h2>5. Comments / General Comments</h2></header><textarea name="generalComments" rows="5">${esc(normalized.generalComments)}</textarea></section>${gameEvaluationSummary(normalized)}<footer class="evaluation-form-actions"><button type="button" data-action="clear-game-evaluation">${icon('i-close')}Clear Form</button><button type="submit" class="is-primary">${icon('i-check')}${normalized.id ? 'Update Evaluation' : 'Save Evaluation'}</button></footer></form>`;
  }

  function gameEvaluationHistory() {
    const query = gamesUi.query.trim().toLowerCase();
    const records = gameEvaluations().filter((record) => !query || [record.officialName, record.evaluatorName, record.site, record.homeTeam, record.visitingTeam, record.levelGender].join(' ').toLowerCase().includes(query)).sort((a,b) => `${b.evaluationDate}${b.updatedAt}`.localeCompare(`${a.evaluationDate}${a.updatedAt}`));
    ...
    return `<section class="game-evaluation-history"><header><div><h1>Saved Evaluations</h1><p>Open, revise, download, or remove evaluations created from actual game and official records.</p></div><label>${icon('i-search')}<input type="search" value="${esc(gamesUi.query)}" data-game-evaluation-search aria-label="Search saved evaluations"></label></header>${pageItems.length ? `<div class="evaluation-history-grid">${pageItems.map((record) => `<article><div class="evaluation-history-score"><strong>${record.overallScore ? record.overallScore.toFixed(1) : '—'}</strong><span>Overall</span></div><div><p>${record.evaluationDate ? esc(fmtDate(record.evaluationDate)) : 'Date not entered'}</p><h2>${esc(record.officialName)}</h2><strong>${esc(gameEvaluationGameLabel(record))}</strong><small>Evaluator: ${esc(record.evaluatorName)}</small></div><footer><button type="button" data-action="edit-game-evaluation" data-id="${esc(record.id)}">${icon('i-edit')}Edit</button><button type="button" data-action="download-game-evaluation" data-id="${esc(record.id)}">${icon('i-download')}Download</button><button type="button" data-action="delete-game-evaluation" data-id="${esc(record.id)}">${icon('i-trash')}Delete</button></footer></article>`).join('')}</div><nav class="evaluation-history-pagination" aria-label="Saved evaluations pages"><button type="button" data-action="game-evaluation-page" data-page="${gamesUi.page - 1}" ${gamesUi.page <= 1 ? 'disabled' : ''}>${icon('i-chevron')}Previous</button><span>Page ${gamesUi.page} of ${pageCount}</span><button type="button" data-action="game-evaluation-page" data-page="${gamesUi.page + 1}" ${gamesUi.page >= pageCount ? 'disabled' : ''}>Next${icon('i-chevron')}</button></nav>` : `<div class="game-evaluation-empty">${icon('i-star')}<h2>${gameEvaluations().length ? 'No evaluations match this search.' : 'No evaluations have been saved.'}</h2><p>${gameEvaluations().length ? 'Clear the search to view saved records.' : 'Complete the Basketball Referee Evaluation Form to create the first record.'}</p><button type="button" data-action="set-games-section" data-section="evaluation">Open Evaluation Form</button></div>`}</section>`;
  }

  function renderGames() {
    const editing = gamesUi.editingId ? gameEvaluations().find((record) => record.id === gamesUi.editingId) : null;
    const reportEditing = gamesUi.reportEditingId ? gameReports().find((record) => record.id === gamesUi.reportEditingId) : null;
    const reportGame = gamesUi.reportGameId ? (state.masterGames || []).find((game) => game.id === gamesUi.reportGameId) : null;
    const reportRecord = reportEditing || (reportGame ? gameReportSeedFromGame(reportGame) : normalizeGameReport({}));
    const content = gamesUi.section === 'reviews' ? gameEvaluationReviewsDashboard() : gamesUi.section === 'history' ? gameEvaluationHistory() : gamesUi.section === 'evaluation' ? gameEvaluationForm(editing || {}) : gamesUi.section === 'report' ? gameReportForm(reportRecord) : gameReportList();
    return `<section class="games-evaluation-page">${routeHeader('Games', 'Create evaluations, review completed performance records, and submit post-game reports from actual published games.')} ${gameEvaluationTasks()}${content}</section>`;
  }
  ...
  function populateGameEvaluationFromGame(form, gameId) {
    ...
    set('site', game.gymName || game.location || game.address);
    ...
    const partners = masterAllRoleKeys(game).map((key) => game[key]).filter(Boolean).map(masterCrewName).filter(Boolean);
    set('partners', partners.join(', '));
  }
  ...
  function gameEvaluationDocumentHtml(record) {
    const groups = GAME_EVALUATION_GROUPS.map((group) => `<section><h2>${esc(group.title)}</h2><table><thead><tr><th>Criterion</th><th>Score</th><th>Rating</th><th>Comments</th></tr></thead><tbody>${group.criteria.map(([key,label]) => { const fieldKey = `${group.key}.${key}`; const score = record.scores[fieldKey] || ''; return `<tr><td>${esc(label)}</td><td>${esc(score)}</td><td>${esc(GAME_EVALUATION_SCORE_LABELS[String(score)] || '')}</td><td>${esc(record.comments[fieldKey] || '')}</td></tr>`; }).join('')}</tbody></table></section>`).join('');
    return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(record.officialName)} Basketball Referee Evaluation</title><style>body{font-family:Arial,sans-serif;max-width:1100px;margin:36px auto;padding:0 24px;color:#161616}header{border-bottom:5px solid #a50000;padding-bottom:18px}h1{font-size:34px;margin:0 0 8px;text-transform:uppercase}h2{margin-top:30px;padding:8px 10px;background:#151515;color:#fff}dl{display:grid;grid-template-columns:190px 1fr;gap:8px 18px}dt{font-weight:700}dd{margin:0}table{width:100%;border-collapse:collapse}th,td{border:1px solid #aaa;padding:8px;text-align:left;vertical-align:top}th{background:#eee}.score{font-size:24px;font-weight:700;color:#a50000}.summary{white-space:pre-wrap}</style></head><body><header><p>Raising The Bar Officiating</p><h1>Basketball Referee Evaluation Form</h1><div class="score">Overall Score: ${record.overallScore ? record.overallScore.toFixed(2) : 'Not calculated'}</div></header><dl><dt>Official</dt><dd>${esc(record.officialName)}</dd><dt>Evaluator</dt><dd>${esc(record.evaluatorName)}</dd><dt>Date</dt><dd>${esc(record.evaluationDate ? fmtDate(record.evaluationDate) : '')}</dd><dt>Game</dt><dd>${esc(gameEvaluationGameLabel(record))}</dd><dt>Site</dt><dd>${esc(record.site)}</dd><dt>Level / Gender</dt><dd>${esc(record.levelGender)}</dd><dt>Partners</dt><dd>${esc(record.partners)}</dd></dl>${groups}<h2>General Comments</h2><p class="summary">${esc(record.generalComments)}</p><h2>Performance Summary</h2><dl><dt>Strengths</dt><dd class="summary">${esc(record.strengths)}</dd><dt>Areas for Improvement</dt><dd class="summary">${esc(record.areasForImprovement)}</dd><dt>Overall Summary</dt><dd class="summary">${esc(record.overallSummary)}</dd></dl><h2>Signatures</h2><dl><dt>Evaluator Signature</dt><dd>${esc(record.evaluatorSignature)}</dd><dt>Official Acknowledgement</dt><dd>${esc(record.officialAcknowledgement)}</dd><dt>Date</dt><dd>${esc(record.signatureDate ? fmtDate(record.signatureDate) : '')}</dd></dl></body></html>`;
  }
  ...
  moduleView.addEventListener('submit', async (event) => {
    ...
    if (type === 'game-report') {
      const saveMode = event.submitter?.dataset.reportSaveMode === 'draft' ? 'draft' : 'submit';
      const linkedGame = (state.masterGames || []).map(normalizeMasterGame).find((game) => game.id === data.linkedGameId);
      if (!linkedGame) { showToast('Select an actual published game before saving the report.'); return; }
      if (!gameReportIsEligible(linkedGame) && !gameReportForGame(linkedGame.id)) { showToast(gameReportEligibilityText(linkedGame)); return; }
      const existing = gameReports().find((record) => record.id === data.id) || gameReportForGame(linkedGame.id) || null;
      const collectSide = (prefix, keys) => Object.fromEntries(keys.map((key) => [key, data[`${prefix}__${key}`] === '' || data[`${prefix}__${key}`] == null ? '' : Math.max(0, Number(data[`${prefix}__${key}`]) || 0)]));
      const scores = { home: collectSide('score__home', [...GAME_REPORT_PERIODS.map(([key]) => key), 'final']), away: collectSide('score__away', [...GAME_REPORT_PERIODS.map(([key]) => key), 'final']) };
      scores.home.final = GAME_REPORT_PERIODS.reduce((sum,[key]) => sum + (Number(scores.home[key]) || 0), 0);
      scores.away.final = GAME_REPORT_PERIODS.reduce((sum,[key]) => sum + (Number(scores.away[key]) || 0), 0);
      const fouls = { home: collectSide('foul__home', GAME_REPORT_FOUL_ROWS.map(([key]) => key)), away: collectSide('foul__away', GAME_REPORT_FOUL_ROWS.map(([key]) => key)) };
      const incidents = [...form.querySelectorAll('[data-game-report-incident]:checked')].map((input) => input.value);
      const now = new Date().toISOString();
      if (saveMode === 'submit') {
        if (!data.gameStatus || !String(data.crewChiefSignature || '').trim()) { showToast('Select the final game status and enter the crew-chief signature before submitting.'); return; }
        const requiredPeriodFields = ['q1','q2','q3','q4'];
        if (requiredPeriodFields.some((key) => scores.home[key] === '' || scores.away[key] === '')) { showToast('Enter both teams’ Q1 through Q4 scores before submitting.'); return; }
      }
      const id = existing?.id || uid('game-report');
      const record = normalizeGameReport({ ...(existing || {}), ...data, id, linkedGameId: linkedGame.id, reportCode: existing?.reportCode || '', scores, fouls, incidents, overtimePlayed: form.elements.overtimePlayed.checked, delayInGame: form.elements.delayInGame.checked, status: saveMode === 'submit' ? 'Submitted' : 'Draft', submittedAt: saveMode === 'submit' ? now : existing?.submittedAt || '', createdAt: existing?.createdAt || now, updatedAt: now });
      record.reportCode = gameReportCode(record);
      state.gameReports = Array.isArray(state.gameReports) ? state.gameReports : [];
      upsert(state.gameReports, record);
      const masterGame = (state.masterGames || []).find((game) => game.id === linkedGame.id);
      if (masterGame) { masterGame.gameReportId = record.id; masterGame.gameReportStatus = record.status; if (saveMode === 'submit') masterGame.completedAt = masterGame.completedAt || now; masterGame.updatedAt = now; }
      saveState();
      if (saveMode === 'submit') {
        const slug = [record.gameDate, record.homeTeam, record.awayTeam].filter(Boolean).join('-').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') || record.id;
        const blob = new Blob([gameReportHtml(record)], { type: 'text/html' });
        const documentRecord = await dbPutDocumentBlob(blob, { name: `${slug}-basketball-game-report.html`, documentCategory: 'game-reports', status: 'Completed', description: `Post-game report for ${gameReportGameLabel(record)}.`, tags: ['Game Report','Basketball',record.homeTeam,record.awayTeam].filter(Boolean), source: 'game-report', sourceId: record.id });
        logDocumentActivity(normalizeDocumentRecord(documentRecord), existing?.status === 'Submitted' ? 'Updated' : 'Generated');
      }
      gamesUi.reportEditingId = ''; gamesUi.reportGameId = ''; gamesUi.section = 'reports'; gamesUi.reportPage = 1;
      notify(saveMode === 'submit' ? 'Basketball game report submitted.' : 'Basketball game report draft saved.');
    ...
    if (type === 'game-evaluation') {
      ...
      if (!officialName || !evaluatorName || !data.evaluationDate || !data.evaluatorSignature || !data.signatureDate) { showToast('Complete the required official, evaluator, date, and signature fields.'); return; }
      ...
      if (!Object.keys(scores).length) { showToast('Enter at least one performance score before saving the evaluation.'); return; }
      const existing = gameEvaluations().find((item) => item.id === data.id) || null;
      const now = new Date().toISOString();
      const overallScore = gameEvaluationAverage(scores);
      const record = normalizeGameEvaluation({ ...(existing || {}), id: existing?.id || uid('game-evaluation'), linkedGameId: data.linkedGameId, officialId: data.officialId, evaluatorId: data.evaluatorId, officialName, evaluatorName, evaluationDate: data.evaluationDate, site: data.site, homeTeam: data.homeTeam, visitingTeam: data.visitingTeam, levelGender: data.levelGender, partners: data.partners, scores, comments, generalComments: data.generalComments, strengths: data.strengths, areasForImprovement: data.areasForImprovement, overallSummary: data.overallSummary, tracking, evaluatorSignature: data.evaluatorSignature, officialAcknowledgement: data.officialAcknowledgement, signatureDate: data.signatureDate, overallScore, createdAt: existing?.createdAt || now, updatedAt: now });
      state.gameEvaluations = Array.isArray(state.gameEvaluations) ? state.gameEvaluations : [];
      upsert(state.gameEvaluations, record);
      const reviewRecord = { id: `evaluation-review:${record.id}`, game: gameEvaluationGameLabel(record), date: record.evaluationDate, evaluator: record.evaluatorName, overall: overallScore.toFixed(2), mechanics: '', judgment: '', communication: '', comments: record.overallSummary || record.generalComments, sourceEvaluationId: record.id };
      upsert(state.reviews, reviewRecord);
      saveState();
      const safeOfficial = officialName.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') || 'official';
      const blob = new Blob([gameEvaluationDocumentHtml(record)], { type: 'text/html' });
      const documentRecord = await dbPutDocumentBlob(blob, { name: `${safeOfficial}-basketball-referee-evaluation-${record.evaluationDate || 'undated'}.html`, documentCategory: 'evaluations', status: 'Completed', description: `Basketball referee evaluation for ${officialName}.`, tags: ['Evaluation', 'Basketball', officialName], source: 'game-evaluation', sourceId: record.id });
      logDocumentActivity(normalizeDocumentRecord(documentRecord), existing ? 'Updated' : 'Generated');
      gamesUi.editingId = ''; gamesUi.section = 'reviews'; gamesUi.page = 1; gamesUi.season = gameEvaluationSeasonKey(record.evaluationDate) || gamesUi.season;
      notify(existing ? 'Basketball referee evaluation updated.' : 'Basketball referee evaluation saved.');
      await renderRoute('games');
      return;
    }
  ...
  moduleView.addEventListener('input', async (event) => {
    if (event.target.matches('[data-game-report-score],[data-game-report-time]')) { updateGameReportCalculatedFields(event.target.form); return; }
    ...
    if (event.target.matches('[data-game-report-search]')) {
      gamesUi.reportQuery = event.target.value; gamesUi.reportPage = 1; const value = event.target.value;
      await renderRoute('games'); const input = moduleView.querySelector('[data-game-report-search]'); if (input) { input.focus(); input.value = value; input.setSelectionRange(value.length, value.length); }
      return;
    }
  ...
  moduleView.addEventListener('change', async (event) => {
    if (event.target.matches('[data-game-evaluation-season]')) { gamesUi.season = event.target.value || 'all'; await renderRoute('games'); return; }
    if (event.target.matches('[data-game-report-link]')) { populateGameReportFromGame(event.target.form, event.target.value); return; }
    if (event.target.matches('[data-game-report-score],[data-game-report-time]')) { updateGameReportCalculatedFields(event.target.form); return; }
    ...
    if (event.target.matches('[data-game-evaluation-official]')) { const person = masterScheduleRoster().find((item) => item.id === event.target.value); const input = event.target.form?.elements?.namedItem('officialName'); if (input && person) input.value = person.name; return; }
  ...
  moduleView.addEventListener('click', async (event) => {
    ...
    if (gameEvaluationAction) {
      ...
      if (action === 'set-games-section') { const requested = gameEvaluationAction.dataset.section; gamesUi.section = ['reviews','history','evaluation','reports','report'].includes(requested) ? requested : 'reviews'; if (gamesUi.section === 'evaluation' && !id) gamesUi.editingId = ''; if (gamesUi.section === 'reports') { gamesUi.reportEditingId = ''; gamesUi.reportGameId = ''; gamesUi.reportPage = 1; } gamesUi.page = 1; await renderRoute('games'); return; }
      ...
      if (action === 'open-published-games-from-reports') { assignmentUi.section = 'published'; publishedGamesUi.detailId = ''; if (location.hash !== '#assignments') history.pushState(null,'','#assignments'); await renderRoute('assignments'); return; }
      if (action === 'open-game-report') { const sourceGame = (state.masterGames || []).find((item) => item.id === id); const game = sourceGame ? normalizeMasterGame(sourceGame) : null; if (!game) { showToast('Published game not found.'); return; } const report = gameReportForGame(game.id); if (!report && !gameReportIsEligible(game)) { showToast(gameReportEligibilityText(game)); return; } if (!report && !sourceGame.completedAt) { if (!confirm('Confirm that this game has finished and open the post-game report?')) return; sourceGame.completedAt = new Date().toISOString(); sourceGame.updatedAt = sourceGame.completedAt; saveState(); } gamesUi.reportEditingId = report?.id || ''; gamesUi.reportGameId = game.id; gamesUi.section = 'report'; if (location.hash !== '#games') history.pushState(null,'','#games'); await renderRoute('games'); return; }
      if (action === 'back-to-game-reports') { gamesUi.reportEditingId = ''; gamesUi.reportGameId = ''; gamesUi.section = 'reports'; await renderRoute('games'); return; }
      if (action === 'game-report-page') { gamesUi.reportPage = Math.max(1, Number(gameEvaluationAction.dataset.page) || 1); await renderRoute('games'); return; }
      if (action === 'download-game-report') { const record = gameReports().find((item) => item.id === id); if (!record) return; const slug = [record.gameDate,record.homeTeam,record.awayTeam].filter(Boolean).join('-').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') || record.id; downloadText(`${slug}-basketball-game-report.html`, gameReportHtml(record), 'text/html'); return; }
      if (action === 'delete-game-report') { const record = gameReports().find((item) => item.id === id); if (!record || !confirm(`Delete the game report for ${gameReportGameLabel(record)}?`)) return; state.gameReports = state.gameReports.filter((item) => item.id !== id); const game = (state.masterGames || []).find((item) => item.id === record.linkedGameId); if (game) { game.gameReportId = ''; game.gameReportStatus = ''; } const docs = await dbListAllFiles(); for (const file of docs.filter((item) => item.category === 'documents' && item.source === 'game-report' && String(item.sourceId || '') === id)) await dbDeleteFile(file.id); saveState(); gamesUi.section = 'reports'; await renderRoute('games'); showToast('Game report deleted.'); return; }
