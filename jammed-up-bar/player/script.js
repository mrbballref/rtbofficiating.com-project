...
async function toggleRecording() {
  ...
  try {
    ...
    mediaRecorder.onstop = () => {
      ...
      const item = { id: crypto.randomUUID(), title: `Screen Recording ${new Date().toLocaleTimeString()}`, description: 'Recorded with the built-in player recording control.', creatorName: 'Current User', creatorRole: 'Recorder', category: 'Recording', tags: 'recording', url, thumb: '', likes: 0, dislikes: 0, liked:false, disliked:false, saved:false, createdAt: Date.now(), duration: 'REC' };
...
// The Jammed Up Bar! integration: load a verified media URL passed by the platform.
(() => {
  try {
    const params = new URLSearchParams(location.search);
    const src = params.get('src');
    if (!src) return;
    const title = params.get('title') || 'Published Video';
    const item = {
      url: src,
      title,
      description: params.get('description') || '',
      creatorName: params.get('creator') || 'The Jammed Up Bar!',
      creatorRole: 'Raising The Bar Officiating Media',
